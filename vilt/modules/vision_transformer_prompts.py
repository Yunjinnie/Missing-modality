...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        print("before patch_embed", x.size())
        # torch.Size([1, 3, 608,, 320])
        x = self.proj(x)
        print("after proj", x.size())#[1, 768,  19, 10])
        return x
...
#             trunc_normal_(self.mask_token, std=0.02)
    ...
    def visual_embed(self, _x, max_image_len=200, mask_it=False):
        _, _, ph, pw = self.patch_embed.proj.weight.shape
        # print("before patch_embed", _x.size()) #[1, 3, 480, 384])
        x = self.patch_embed(_x)
        # print("after patch_embed", x.size()) #([1, 768, 15, 12])
        ...
        # print("x size", x.size()) #([1, 192, 768])

        x = torch.cat((cls_tokens, x), dim=1)
        # print("before pos_embed", self.pos_embed.size())# torch.Size([1, 145, 768
        pos_embed = torch.cat(
            (self.pos_embed[:, 0, :][:, None, :].expand(B, -1, -1), pos_embed), dim=1
        )
        x = x + pos_embed
        # print("after pos_embed", pos_embed.size()) #torch.Size([1, 191, 768])
