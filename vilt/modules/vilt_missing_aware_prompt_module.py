...
# 
        ...
        self.learnt_p = self.hparams.config["learnt_p"] if self.hparams.config["prompt_use"] else False
        self.prompt_layers = self.hparams.config["prompt_layers"] if self.hparams.config["prompt_use"] else []
        ...
        # self.missing_text_prompt = None
        # self.missing_img_prompt = None

        if not self.hparams.config["full_finetune"]:
    ...
    def infer(
        ...
        if image_embeds is None and image_masks is None:
            ...
            print("image size: ", img.size()) #torch.Size([1, 3, 512, 384]) #image size:  torch.Size([1, 3, 576, 384])
            (
                image_embeds,
                image_masks,
                patch_index,
            ...
            # print("after image size: ", image_embeds.size())  torch.Size([1, 191, 768])
...
#                     image_embeds[idx,1:].fill_(1)
        ...
        if self.hparams.config["prompt_use"]:
