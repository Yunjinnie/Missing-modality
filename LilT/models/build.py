...
from LilT.models.tokenization_bert import BertTokenizer
from torch import nn
from functools import partial
from LilT.models.vit import VisionTransformer, interpolate_pos_embed
from LilT.models.deit_v2 import vit_models, Layer_scale_init_Block
from LilT.models.med import BertLayer
import torch
...
def text_encoder(config: DictConfig, text_encoder: str, adapter_append: bool):
    if config["pretrained_text"]:
        try:
            ...
            # bert_config.max_position_embeddings = config["max_text_len"]
            ...
            if config["max_text_len"] == 1024:
                pos_emb_weight = model.embeddings.position_embeddings.weight
                model.embeddings.position_embeddings = nn.Embedding(1024,1024)
                pos_emb = torch.nn.functional.interpolate(
                    pos_emb_weight.view(1, 1, 512, 1024), size=(config["max_text_len"], 1024) ,mode='bilinear').squeeze()
                model.embeddings.position_embeddings.weight = nn.Parameter(pos_emb)
                model.embeddings.position_ids = torch.Tensor(range(config["max_text_len"])).long().view(1,-1)
            return model
